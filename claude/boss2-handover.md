...
### 9.1 사용자 확정 작업 목록 (2026-07-20 지시, 구현 방식 미확정 - 착수 전 방향 협의 필요)

- [ ] 보스 이펙트(후광/잔상 등)로 보스 비주얼 꾸미기
- [ ] 4페이즈 보스 사망 연출
- [ ] 낙인 7스택 즉사("낙인사") 전용 연출 - 위 `Boss2BrandController.CoBrandLoop` TODO 훅과 동일 항목
- [ ] 제단(Altar) 홀드 프로그레스 바 아트 적용 - 위 `HoldUI` 항목과 동일 항목
- [ ] 낙인 스택 UI(`Panel_BrandCount`) 비주얼 꾸미기 - 위 항목과 동일 항목
- [ ] 보스 근접 공격 패턴 추가 (현재 `BossFloatMovement`의 몸통박치기 돌진 1종뿐)
- [ ] 4페이즈 진입 시 애니메이션을 "Scream"(비명)으로 교체 - 현재 `Boss2.controller`엔 Idle State만 있어 애니메이터 State 추가부터 필요
