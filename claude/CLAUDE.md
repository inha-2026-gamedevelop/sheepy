...
| `12.Item/` | 포션(회복 소비) 드랍/자석픽업/카운트 - PotionManager(IRewindable, 자동 생성), PotionPickupPool, MapTopItemPickup |
...
- 밸런싱 수치는 ScriptableObject DB로 관리: 루트 `GameDatabaseSO`(`08.Data/Resources/GameDB.asset`, Resources 자동 로드)가 `PlayerDataSO`/`BossDataSO`/`TimeDataSO`/`PotionDataSO`(`08.Data/Player|Boss|Time|Potion/*DB.asset`)를 묶고, 정적 접근자 `GameDB.Player.MoveSpeed` 형태로 읽는다 (`Minsung.Common.Data`).
