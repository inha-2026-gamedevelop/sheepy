...
- **Unity**: 6000.4.7f1 (2D URP) / **C#** / **C# FSM**(몬스터/보스 AI) / **Cinemachine 3.1**(포커스 카메라)
...
5. **AI 판단은 C# FSM으로 구현**: 일반 몬스터는 `MonsterState`(Patrol/Chase/Attack), 보스는 `BossState`/`Phase1~4State`가 판단을 전담한다. 상태 추가 시 전이 우선순위와 리와인드 중 정지 규칙을 함께 확인한다.
6. **리와인드 잠금은 핸들로 관리**: `RewindManager.AcquireRewindLock(owner)`의 반환값을 보유하고 해당 주체가 `Dispose()`한다. `SetRewindEnabled(bool)` 같은 전역 bool 토글을 새로 만들지 않는다.
7. **싱글톤 공통 베이스 사용** — DontDestroyOnLoad는 `Utility/PersistentSingleton<T>`, 씬 로컬은 `Utility/SceneSingleton<T>` 상속. Awake 직접 구현 금지, `OnSingletonAwake()` 오버라이드.
8. **이모티콘 사용 금지** - `→` = `->`,  `←` = `<-`, `—` = `-` 등 사람이 잘 사용하지 않는 특수문자 혹은 기호들을 사람이 자주 사용하는것으로 변경.
9. **주석 뒤 . 금지** - 주석 뒤에는 .을 붙이지 말고 이 행동이 무엇을 의미하는지 간단히만 설명할것.
10. **샘플/참고 구현은 정식 폴더로 승격해서 쓴다**: UI 진행바 등 참고 구현이 필요하면 정식 폴더(`06.UI` 등)에 프로덕션 코드로 만든다. (과거 `Ex/` 샘플 폴더는 2026-07-18 완전 제거됨 - 프로덕션은 전부 `Minsung.*`)
11. **진행바(HP바 등)는 `Slider` 컴포넌트로 구현**. `Image.fillAmount` 대신 `Slider.value`(0~1 정규화)를 쓴다. (`BossHealthBarUI` 기준)
12. **커밋 메시지는 `claude/commit-convention.md` 규칙**(`type: Scope 한국어 제목`)을 따른다. 세션 간 인수인계(재개 지점/미완 작업)는 과거 `claude/PLAN.md` 대신 **영속 메모리**(개인 `MEMORY.md`)에 기록한다 - `claude/PLAN.md`는 2026-07-22 제거됨.
13. **UI 오브젝트 이름은 기본 On/Off 상태를 접미사로 표기**: 기본적으로 꺼져 있는 상태로 시작하면 이름 뒤에 `[OFF]`, 기본적으로 켜져 있어야 하면 `[ON]`을 붙인다 (예: `Caption[OFF]`).
...
몬스터  : MonsterState FSM (Patrol/Chase/Attack)   -- 판단 담당, Request* 호출
    v
...
- 보스는 **씬 분할 구조**: Boss1(`BossController` 계열, Map2, 1~2P)은 피통/페이즈 + `BossState` 상태 패턴 + 공용 `BossEmotionController`(감정 변조·결정 로그)를 유지하고, Boss2(`03.Boss/Boss2/`, Map3, 3~4P)는 독립 피통(`Boss2Health`)·부유 이동·전용 패턴을 갖는 별도 시스템(팀 컨벤션상 진욱 코드는 네임스페이스 없음, `Minsung.Boss` 공용 인프라만 소비). 1~3P 구현 완료, 4P 구현 진행 중. 상세는 `claude/boss.md`

### 폴더 (Assets/01.Scripts/, 번호 접두사) — 총 180개 스크립트 (전부 `Minsung.*`, 샘플 폴더 `Ex/`는 2026-07-18 제거)

| 폴더 | 내용 |
|---|---|
| `00.Common/` | GameManager, SaveManager/SaveData, RespawnManager, PauseController, OneWayPlatform, ManagedObjectManager, DamageSource/IDamageable, Constants(계약값), **Data(GameDB + PlayerDataSO/BossDataSO/TimeDataSO/LpDataSO/PotionDataSO 밸런싱 DB + `Boss2DataSO`(GameDB 트리 밖 독립 DB))**, Utility(PersistentSingleton/SceneSingleton/UtilCoroutine) |
| `01.Player/` | 코디네이터(PlayerController) + Input/Movement/Combat/Interaction/Rewind/StatusEffect 컴포넌트, 하트 체력, 오브 공격(발광/궤적 비주얼), 애니메이터, HUD, 상호작용 센서, 사망/리스폰(DeathZone·RespawnPoint·PlayerRetireZone), 피격 피드백, 사운드 |
| `02.Monster/` | 몬스터 몸통/체력/애니메이터(사망 모션 포함), FSM(순찰·추격·공격) |
| `03.Boss/` | **Boss1**(민성, Map2 1~2P): 단일 피통 컨트롤러(씬 담당 페이즈 구간 분할 + 아웃트로 전환), Phase1~4State(FSM), 공용 감정(BossEmotionController), 근접유닛(본체/분신), 낙뢰/해저드 풀, DamageHazard. **Boss2/**(진욱, Map3 3~4P 별도 시스템): 부유 이동(BossFloatMovement)/체력(Boss2Health)/원거리 패턴 3종/감정/낙인·제단/공간찢기·손아귀. 상세는 `claude/boss.md` |
| `04.TimeSystem/` | RewindManager(주체별 RewindLockHandle), RingBuffer, 커맨드(Move/Attack/Interact/Anim/Tick), 분신, 슬로우 |
...
| `12.Item/` | LP(수집 재화)/포션(회복 소비) 드랍/자석픽업/카운트 - LpManager·PotionManager(IRewindable, 자동 생성), LpPickupPool·PotionPickupPool, MapTopItemPickup |
...
- 밸런싱 수치는 ScriptableObject DB로 관리: 루트 `GameDatabaseSO`(`08.Data/Resources/GameDB.asset`, Resources 자동 로드)가 `PlayerDataSO`/`BossDataSO`/`TimeDataSO`/`LpDataSO`/`PotionDataSO`(`08.Data/Player|Boss|Time|Lp|Potion/*DB.asset`)를 묶고, 정적 접근자 `GameDB.Player.MoveSpeed` 형태로 읽는다 (`Minsung.Common.Data`).
- MonoBehaviour 필드 초기화식에서 `GameDB` 호출 금지 (Resources.Load 제약) - Awake 이후 사용. 컴포넌트에 밸런싱 SerializeField 미러를 두지 않는다.
- 수치 튜닝은 DB 에셋 인스펙터에서. 몬스터(`ENEMY_*`)는 FSM 기본값/배치별 튜닝 전제로 Constants 유지.
...
| `claude/boss.md` | 보스(Azathoth) 통합 기획+구현 문서 - 팀 분담(민성 1~2P/진욱 3~4P), 공통 규칙/감정, 페이즈별 상세, Boss2 구현 구조/코드 흐름/검증 상태/알려진 이슈/함정 체크리스트. 보스 작업 시 필독 (구 `boss-design.md`/`boss2-handover.md`/`boss2-handover-prompt-next.md`/`boss2-code-guide.md` 통합, 2026-07-20) |
...
- [ ] **커밋 전 이번에 수정/생성한 파일을 전부 `claude/coding-convention.md` 기준으로 재검토** - 어긋난 부분은 커밋 전에 직접 수정(중괄호/전위증감/조건 괄호/필드 정렬/using 배너/주석 마침표/네임스페이스 등). 위반이 없으면 그대로 진행
