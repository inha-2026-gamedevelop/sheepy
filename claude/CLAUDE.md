...
10. **`Ex/` 폴더는 샘플/예제 코드**(네임스페이스 없음). 프로덕션 코드(`Minsung.*`)가 `Ex`를 참조하면 안 된다. UI 진행바 등 참고 구현은 정식 폴더(`06.UI` 등)로 승격해서 쓴다.
11. **진행바(HP바 등)는 `Slider` 컴포넌트로 구현**. `Image.fillAmount` 대신 `Slider.value`(0~1 정규화)를 쓴다. (`BossHealthBarUI` 기준)
12. **커밋 전 `claude/PLAN.md`를 최신 상태로 갱신**하고, **커밋 메시지는 `claude/commit-convention.md` 규칙**(`type: Scope 한국어 제목`)을 따른다.
...
- 보스는 각자 `BossController` 계열(피통/페이즈) + `BossState` 상태 패턴을 유지하고, 공용 `BossEmotionController`가 감정 변조(반사/낙뢰/혼란)와 결정 로그를 소유한다. Phase1~3 구현, Phase4는 2페이즈 패턴 임시 유지

### 폴더 (Assets/01.Scripts/, 번호 접두사) — 총 135개 스크립트 (전부 `Minsung.*`, 샘플 폴더 `Ex/`는 2026-07-18 제거)

| 폴더 | 내용 |
|---|---|
| `00.Common/` | GameManager, SaveManager, DamageSource/IDamageable, Constants(계약값), **Data(GameDB + PlayerDataSO/BossDataSO/TimeDataSO/LpDataSO/PotionDataSO - 밸런싱 DB)**, Utility(PersistentSingleton/SceneSingleton/UtilCoroutine) |
| `01.Player/` | 코디네이터(PlayerController) + Input/Movement/Combat/Interaction/Rewind/StatusEffect 컴포넌트, 하트 체력, 오브 공격(발광/궤적 비주얼), 애니메이터, HUD, 상호작용 센서 |
| `02.Monster/` | 몬스터 몸통/체력/애니메이터(사망 모션 포함), FSM(순찰·추격·공격) |
| `03.Boss/` | 단일 피통 컨트롤러(페이즈 구간별 씬 분할 + 아웃트로 전환), Phase1~4State(FSM), 공용 감정(BossEmotionController), 근접유닛(본체/분신), 낙뢰/해저드 풀, DamageHazard |
...
| `12.Item/` | LP(수집 재화)/포션(회복 소비) 드랍/자석픽업/카운트 - LpManager·PotionManager(IRewindable, 자동 생성), LpPickupPool·PotionPickupPool |
...
| `claude/PLAN.md` | **구현 목록 + 인수인계 (2026-07-18 최신)** — 완료/우선순위별 남은 작업 / 리팩토링 이력 / 씬 구성·신규 기능·버그 수정·주의사항·검증 상태. 작업을 이어받으면 필독. **커밋 전 반드시 갱신** (`HANDOVER.md`는 더 이상 사용하지 않음 - 내용은 PLAN.md로 통합) |
