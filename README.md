...
- **타임리와인드(`D`)** - 최근 N초간의 행동을 역재생으로 되감은 뒤, 그 구간을 정방향으로 재연하는 분신을 소환한다.
- **분신 소환** - 되감기 종료 시 기록된 커맨드 클립을 그대로 재생하는 분신이 등장한다(최대 3개, `F`로 전체 삭제).
...
| `W` | 슬로우모션 |
| `E` | 상호작용 (레버, 라디오 등) |
| `R` | 공격 (범위 안에 적이 있으면 오브 발사, 없으면 근접 히트박스) |
| `S` | 특수키 (4페이즈 공간찢기 회피) |
| `D` | 타임리와인드 + 분신 소환 |
| `F` | 분신 전체 제거 |
