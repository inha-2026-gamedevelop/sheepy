...
namespace Minsung.Boss2
{
    ...
    public class Boss2DataSO : ScriptableObject
    {
        ...
        ****************************************/
        ...
        [Header("손아귀 (4페이즈 잡기 - 손 히트박스가 날아가 붙잡는다)")]
        [SerializeField] private float   _grabCooldown         = 20f;   // 재시도 대기시간(초)
        [SerializeField, Range(0f, 1f)] private float _grabChance = 0.5f; // 쿨타임마다 실제 시전 확률
        [SerializeField] private float   _grabRange            = 8f;    // 플레이어가 이 거리 안에 있어야 시도(유닛)
        [SerializeField] private float   _grabTelegraphTime    = 0.7f;  // 손 발사 전 예고(점멸) 시간(초) - 회피 창
        [SerializeField] private float   _grabHandSpeed        = 22f;   // 손이 뻗어나가는 속도(유닛/초)
        [SerializeField] private float   _grabReach            = 9f;    // 손이 최대로 뻗는 거리(유닛) - 여기까지 안 닿으면 회피 성공
        [SerializeField] private Vector2 _grabHitboxSize       = new Vector2(1.4f, 1.4f); // 손 잡기 판정 크기(유닛)
        [SerializeField] private float   _grabRetractSpeed     = 18f;   // 붙잡은 뒤 손을 보스로 회수하는 속도(유닛/초)
        [SerializeField] private int     _grabDamageHalves     = 2;     // 포박 중 피해 틱마다 하트 차감(반칸 단위 - 한 칸)
        [SerializeField] private float   _grabDamageTickInterval = 1.5f;// 포박 중 피해 틱 간격(초)
        [SerializeField] private int     _grabDamageTickCount    = 3;   // 포박 중 피해 틱 횟수 - 총 포박 지속 = 간격 x 횟수(기본 4.5초)
        [SerializeField] private float   _grabStruggleWindow     = 3f;  // 탈출 입력 집계 창(초) - 이 창 안에 필요 횟수 이상 누르면 탈출
        [SerializeField] private int     _grabStruggleRequiredPresses = 20; // 창 안에서 탈출에 필요한 상호작용 키 입력 횟수
        [SerializeField] private float   _grabThrowForce       = 16f;   // 탈출 실패 시 위로 튕기는 힘(유닛/초) - 연출용, 데미지 없음
        [SerializeField] private float   _grabTelegraphThickness = 0.14f; // 예고선 두께(유닛)
        [SerializeField] private float   _grabTelegraphBlink   = 0.1f;  // 예고선 점멸 주기(초)
        [SerializeField] private Color   _grabTelegraphColor   = new Color(0.8f, 0.1f, 0.9f, 0.55f); // 예고선 색(보라 경고)
        [SerializeField] private Color   _grabHandColor        = new Color(0.75f, 0.2f, 0.95f, 0.9f); // 날아가는 손 색
        ...
        ****************************************/
        ...
        public float   GrabCooldown         => _grabCooldown;
        public float   GrabChance           => _grabChance;
        public float   GrabRange            => _grabRange;
        public float   GrabTelegraphTime    => _grabTelegraphTime;
        public float   GrabHandSpeed        => _grabHandSpeed;
        public float   GrabReach            => _grabReach;
        public Vector2 GrabHitboxSize       => _grabHitboxSize;
        public float   GrabRetractSpeed     => _grabRetractSpeed;
        public int     GrabDamageHalves     => _grabDamageHalves;
        public float   GrabDamageTickInterval => _grabDamageTickInterval;
        public int     GrabDamageTickCount    => _grabDamageTickCount;
        public float   GrabStruggleWindow     => _grabStruggleWindow;
        public int     GrabStruggleRequiredPresses => _grabStruggleRequiredPresses;
        public float   GrabThrowForce       => _grabThrowForce;
        public float   GrabTelegraphThickness => _grabTelegraphThickness;
        public float   GrabTelegraphBlink     => _grabTelegraphBlink;
        public Color   GrabTelegraphColor   => _grabTelegraphColor;
        public Color   GrabHandColor        => _grabHandColor;
