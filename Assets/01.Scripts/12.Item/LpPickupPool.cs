...
namespace Minsung.Item
{
    ...
    public class LpPickupPool
    {
        ...
        ****************************************/
        ...
        private readonly Color _color = new Color(0.4f, 1f, 0.6f); // LP 전용 아트 확보 전 임시 색상 (연두)
        ...
        ****************************************/
        ...
        public LpPickupPool(int size)
        {
            _slots = new PoolSlot[size];
            for (int i = 0; i < size; ++i)
            {
                GameObject go = new GameObject($"LpPickup_{i}");
                go.SetActive(false);
        ...
        ****************************************/
        ...
        /// <summary> 풀 파괴. 소유자(LpManager) 파괴 시 호출 </summary>
        public void Dispose()
        {
            if (_slots == null)
            {
